...
author: Yuga Hikida
institute: yuga.hikida@tu-dortmund.de
fontsize: 11pt
format: 
  beamer:
    incremental: false
...
* Previous studies: migration decision from various factors, both of the channels [@buch2014makes; @hakim2022amenities].
  + No single strong determinants of migration.
 
## Data
 * Focus on % changes in # employees in each region over time.
...
- Code available at <https://github.com/yugahikida/QRE>.
...
- Consider so called direct effect and indirect effect (explained later).
...
\end{align*}
where $X_k = [x_{1k},...,x_{Nk}]^T$.

- Direct effect = impact of $x_{ik}$ on $y_{i}$
  + Diagonal element of $S_k(W)$.
- Indirect effect = impact of $x_{jk}, j \neq i$ on $y_{i}$
...
- Negative direct effect / positive spillover in waste amount.
